...
function createCompatibilityCSS(): string {
  return `
    * { 
    ...
    /* E      const imgData = canvas.toDataURL('image/png');
      pdf.add        });
        ...
        // Restore element positioning
        elementPrepareRestore();
      }imgData, 'PNG', 20, yPosition, imgWidth, imgHeight);
    ...
    }re elements with transparent backgrounds get white */
...
async function bulkExportToPDF(
  ...
  for (let i = 0; i < elements.length; i++) {
    ...
    try {
      ...
      // Prepare element for better positioning
      const elementPrepareRestore = prepareElementForScreenshot(element);
      ...
      try {
        ...
        // Capture element with improved positioning
        const canvas = await html2canvas(element, {
          scale: 1.5,
          backgroundColor: '#ffffff',
          logging: false,
          x: Math.max(0, elementRect.left + scrollX),
          y: Math.max(0, elementRect.top + scrollY),
...
/**
 * Prepare chart container specifically for screenshot with enhanced positioning
 */
function prepareChartContainerForScreenshot(chartElement: HTMLElement): () => void {
  const restoreFunctions: Array<() => void> = [];
  
  // Find chart container and all parent elements
  ...
  // Fix each element in the hierarchy
  elements.forEach(el => {
    ...
    // Apply fixes
    el.style.position = el === chartElement ? 'relative' : 'static';
    el.style.transform = 'none';
    el.style.transformOrigin = 'top left';
    el.style.overflow = 'visible';
    el.style.zIndex = el === chartElement ? '1' : 'auto';
    el.style.filter = 'none';
    
    // Add restore function
    restoreFunctions.push(() => {
      Object.keys(originalStyles).forEach(prop => {
        const value = (originalStyles as any)[prop];
        if (value) {
